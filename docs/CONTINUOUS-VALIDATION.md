...
| G12 | `MATCH_GIT_BASIC_AUTHORIZATION` 404s on the certs repo after delete + recreate. Fine-grained GitHub PATs are pinned to a *repo's database ID*, not its name — when the certs repo is deleted and a new one with identical name is created, the new repo has a fresh ID and the existing PAT loses access. `fastlane match` then dies at the first clone with "Error cloning certificates git repo". This blocks the template's E2E refork test from running unattended (every cycle would require manual PAT scope updates via github.com/settings/tokens). | Document the trade-off in README `Set up signing + ASC` step 4: if you only ever bootstrap once, **"Only select repositories"** scope is fine. If you plan to rerun the E2E refork test, rotate certs repos, or recreate often, choose **"All repositories"** scope (still narrow `Contents` permission, but bound to user/org rather than repo ID — survives recreation). Methodology alternative for the E2E refork test specifically: don't delete the certs repo, just *reset* its branches via force-push. The certs repo's lifecycle is logically separate from the app fork's lifecycle. |
